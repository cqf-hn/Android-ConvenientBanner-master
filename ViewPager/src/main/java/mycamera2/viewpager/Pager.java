...
public class Pager extends View implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public Pager(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.Pager, defStyleAttr, 0);
        int N = a.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.Pager_DefaultScrollAction:
                    scrollAction = a.getInt(attr, AUTO_SCROLL_TO_LEFT);
                    break;
                case R.styleable.Pager_TurnOverLimitValue:
                    turnOverLimitValue = a.getFloat(attr, TDevice.getScreenWidth(getContext()) * 1.0f * 2 / 3);
                    break;
            }
        }
        a.recycle();
    ...
    private void addBitmap(Context context, int resId, final ArrayList<Bitmap> bitmaps) {
        Glide.with(context)
                .load(resId)
                .asBitmap()
                .dontAnimate()
